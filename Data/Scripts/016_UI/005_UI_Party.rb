...
class PokemonPartyBlankPanel < Sprite
  ...
  def initialize(_pokemon, index, viewport = nil)
    super(viewport)
	column_x = [142 + 8, 440 + 8]
    row_y = [16, 130, 244]
    self.x = column_x[index % 2]
    self.y = row_y[index / 2]
...
class PokemonPartyPanel < Sprite
  ...
  def draw_name
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.name, 96 + 11, 22, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
  end
  ...
  def draw_level
    ...
    # "Lv" graphic
    pbDrawTextPositions(@overlaysprite.bitmap,
                         [[_INTL("Lv."), 23 + 5 + 11, 66, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    # Level number
   
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.level.to_s, 47 + 5 + 11, 66, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    pbSetSystemFont(@overlaysprite.bitmap)
  end
  ...
  def draw_gender
    ...
    base_color   = (@pokemon.male?) ? Color.new(0 + 11, 112, 248) : Color.new(232, 32, 16)
    shadow_color = (@pokemon.male?) ? Color.new(120 + 11, 184, 232, 0) : Color.new(248, 168, 184, 0)
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[gender_text, 96 + 11, 46, :left, base_color, shadow_color]])
  end
  ...
  def draw_hp
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[hp_text, 224 + 11, 66, :right, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    # HP bar
    if @pokemon.able?
      ...
      @overlaysprite.bitmap.blt(128 + 11, 52, @hpbar.bitmap, hprect)
    end
  end
  ...
  def draw_status
    ...
    @overlaysprite.bitmap.blt(20 + 5 + 11, 67, @statuses.bitmap, statusrect)
  end

  def draw_shiny_icon
    return if @pokemon.egg? || !@pokemon.shiny?
    pbDrawImagePositions(@overlaysprite.bitmap,
                         [["Graphics/UI/shiny", 80 + 11, 48, 0, 0, 16, 16]])
  end
