...
class Window_PokemonOption < Window_DrawableCommand
  ...
  SEL_NAME_SHADOW_COLOR  = Color.new(192, 32, 40, 0)
  SEL_VALUE_BASE_COLOR   = Color.new(248, 48, 24)
  SEL_VALUE_SHADOW_COLOR = Color.new(192, 32, 40, 0)
...
class PokemonOption_Scene
  ...
  def pbStartScene(in_load_screen = false)
    ...
    @sprites["bg"].bitmap = Bitmap.new("Graphics/Pictures/settingsbg")
    @sprites["bg"].z = 0 # Ensure it's behind everything else
...
option_width = 512
option_x = (Graphics.width - option_width) / 2	
    @sprites["option"] = Window_PokemonOption.new(
      @options, option_x, @sprites["textbox"].height, option_width,  # Starts below the textbox
      Graphics.height - @sprites["textbox"].height             # Fills the remaining height
    )
