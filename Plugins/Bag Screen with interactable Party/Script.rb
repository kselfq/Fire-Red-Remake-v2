...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    textpos = []
    rect = Rect.new(rect.x + 27, rect.y + 18, rect.width - 16, rect.height)
    thispocket = @bag.pockets[@pocket]
    if index == self.itemCount - 1
      ...
      if @sorting && index == self.index
        baseColor   = Color.black
        shadowColor = Color.new(248, 144, 144, 0)
      elsif @party2sel && index == self.index
        baseColor   = self.baseColor
        shadowColor = self.shadowColor
      elsif @selarrow && index == self.index
        baseColor   = Color.black
        shadowColor = Color.new(157, 171, 178, 0)
      end
      textpos.push(
      ...
      if item_data.is_important?
        if @bag.registered?(item)
          pbDrawImagePositions(
            self.contents,
            [["Graphics/UI/Bag Screen with Party/icon_register", rect.x + rect.width - 90, rect.y + 8, 0, 0, -1, 24]]
          )
          showing_register_icon = true
        elsif pbCanRegisterItem?(item)
          pbDrawImagePositions(
            self.contents,
            [["Graphics/UI/Bag Screen with Party/icon_register", rect.x + rect.width - 90, rect.y + 8, 0, 24, -1, 24]]
          )
          showing_register_icon = true
        end
      end
      if item_data.show_quantity? && !showing_register_icon
        ...
        textpos.push([qtytext, xQty - 22, rect.y + 4, :left, baseColor, shadowColor])
      end
    end
...
class PokemonBagPartyBlankPanel < Sprite
  ...
  def initialize(_pokemon,index,viewport=nil)
    ...
    self.x = 0 + (index % 2) * 268
    self.y = (index / 2) * 60 + 16
...
def initialize(pokemon, index, viewport=nil)
    ...
    self.x = 0 + (index % 1) * 268
    self.y = 60 * (index / 1) + 16
  ...
  def refresh
    ...
    if @hpbgsprite && !@hpbgsprite.disposed?
      ...
      if @hpbgsprite.visible
        ...
        @hpbgsprite.x     = self.x + 90
        @hpbgsprite.y     = self.y + 10
        @hpbgsprite.color = self.color
      end
    end
    if @pkmnsprite && !@pkmnsprite.disposed?
      @pkmnsprite.x        = self.x + 32 + 24
      @pkmnsprite.y        = self.y + 24
      @pkmnsprite.color    = self.color
      @pkmnsprite.selected = self.selected
    end
    if @helditemsprite&.visible && !@helditemsprite.disposed?
      @helditemsprite.x     = self.x + 206
      @helditemsprite.y     = self.y + 8
    ...
    if @refreshBitmap
      ...
      if !@pokemon.egg?
        if !@text || @text.length == 0
          # Draw HP numbers
          textpos.push([sprintf("% 3d /% 3d", @pokemon.hp, @pokemon.totalhp), 106, 28, 2, baseColor, Color.new(40, 40, 40, 0), true, Graphics.width]) if !@text || @text.length == 0
        end
          # Draw HP bar
          if @pokemon.hp > 0
            ...
            @overlaysprite.bitmap.blt(76, 11, @hpbar.bitmap, hprect)
          end

        # Draw gender symbol
        if @pokemon.male?
          textpos.push([_INTL("♂"), 210, 10, 0, Color.new(116, 162, 237), outlineColor, true, Graphics.width])
        elsif @pokemon.female?
          textpos.push([_INTL("♀"), 210, 10, 0, Color.new(237, 116, 140), outlineColor, true, Graphics.width])
        end
      ...
      if !@pokemon.egg?
        pbDrawTextPositions(@overlaysprite.bitmap,
                            [[_INTL("Lv."), 186, 28, 2, baseColor, Color.new(40, 40, 40, 0), true, Graphics.width]])
        pbSetSmallFont(@overlaysprite.bitmap)
        pbDrawTextPositions(@overlaysprite.bitmap,
                            [[@pokemon.level.to_s, 206, 28, 2, baseColor, Color.new(40, 40, 40, 0), true, Graphics.width]])
      end
          ...
          if status >= 0
            statusrect = Rect.new(0, 20 * status, 68, 20)
            @overlaysprite.bitmap.blt(152, 25, @statuses.bitmap, statusrect)
          end
        # Draw Pokerus icon
      ...
      if @text && @text.length > 0
        ...
        pbSetSmallFont(@overlaysprite.bitmap)
        pbDrawTextPositions(@overlaysprite.bitmap,
                            [[@text,122, 28, 2, baseColor, Color.new(40, 40, 40, 0), true, Graphics.width]])
      end
    end
    @refreshing = false
  end
...
class PokemonBag_Scene
  ITEMLISTBASECOLOR      = Color.black
  ITEMLISTSHADOWCOLOR    = Color.new(157, 157, 167, 0)
  ITEMTEXTBASECOLOR      = Color.black
  ITEMTEXTSHADOWCOLOR    = ITEMLISTSHADOWCOLOR
  POCKETNAMEBASECOLOR    = Color.black
...
@sprites["overlay"] = BitmapSprite.new(Graphics.width, Graphics.height, @viewport)
    ...
    list_x = 263
    list_width = Graphics.width - list_x - 10 
    
    @sprites["itemlist"] = Window_PokemonBag.new(@bag, @filterlist, lastpocket, list_x, 40, list_width, 72 + ITEMSVISIBLE * 32)
...
def pbRefreshIndexChanged
    ...
    pbDrawTextPositions(
      overlay,
      [[PokemonBag.pocket_names[@bag.last_viewed_pocket - 1], center_x, 18, :center, POCKETNAMEBASECOLOR, POCKETNAMEOUTLINECOLOR, true, Graphics.width]]
    )
