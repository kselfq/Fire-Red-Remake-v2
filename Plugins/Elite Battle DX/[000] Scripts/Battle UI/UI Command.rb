...
#the blaxk box bg behind buttons
    @sprites["bg"] = Sprite.new(@viewport)
    @sprites["bg"].create_rect(@viewport.width,40,Color.new(32,32,32,0))
...
#the color commands buttons    
    h = @btnCmd.height/4
    w = @btnCmd.width/2
    for i in 0...cmds.length
      @sprites["b#{i}"] = Sprite.new(@viewport)
      @sprites["b#{i}"].bitmap = Bitmap.new(@btnEmp.width*2,@btnEmp.height)
#@sprites["b#{i}"].zoom_x = 1.3   # makes button 30% wider
#@sprites["b#{i}"].zoom_y = 1.0   # keep height
...
####################      
      for j in 0...2
@sprites["b#{i}"].bitmap.blt(0, 0, @btnCmd, Rect.new(x, h*i, @btnCmd.width, h))
# unselected 4 commands shadow?
        c = (j > 0) ? @btnCmd.get_pixel(x+8,h*i+8).darken(0.6) : Color.new(51,51,51,0)
#comands color text (battle, bag, party, run)
        pbDrawOutlineText(@sprites["b#{i}"].bitmap,@btnEmp.width*j,11,@btnEmp.width,h,cmds[i],Color.white,Color.new(51,51,51,0),1)
      end

# spacing
button_spacing = 100   # distance between buttons (lower = closer)


#---------------------------------------------
# Custom manual layout (each button position)
#---------------------------------------------
case i
when 0 # FIGHT
  @sprites["b#{i}"].x = 770
...
when 3 # RUN
  ...
  # fallback (in case there’s a debug or safari option)
  @sprites["b#{i}"].x = 800
  @sprites["b#{i}"].y = 350
end
  ...
  def compileCommands(index)
    ...
    default = [_INTL(""), _INTL(""), _INTL(""), _INTL("")]
  ...
  def show
    @sprites["sel"].visible = false
    @sprites["bg"].y -= @sprites["bg"].bitmap.height/1
...
@sprites["sel"].x = @sprites["b#{@index}"].x + Math.sin(Graphics.frame_count * 0.5) * 3
