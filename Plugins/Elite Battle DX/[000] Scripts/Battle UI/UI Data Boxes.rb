...
# Custom override for double battles
if @doublebattle
  case @battler.index
  when 0 # Player left
    return 12
...
if @doublebattle
  ...
  def setUp
    ...
    @sprites["container"].src_rect.height = @showexp ? 26 : 12
  ...
  def updateHpBar
    ...
    textpos = [[str, @sprites["textHP"].bitmap.width + @hpTextX, @hpTextY,1,Color.white,Color.new(0,0,0,0)]]
  ...
  def refresh
    ...
    str += " "
    color = @pokemon.shiny? ? Color.white : Color.white
    ...
    pbDrawOutlineText(@sprites["textName"].bitmap,x-o,3,@sprites["textName"].bitmap.width-40,@sprites["textName"].bitmap.height,str,color,Color.new(0,0,0,0),0)
    # writes the Pokemon's level
    str = "Lv.#{@battler.level}"
    pbDrawOutlineText(@sprites["textName"].bitmap,18+o,3,@sprites["textName"].bitmap.width-40,@sprites["textName"].bitmap.height,str,Color.white,Color.new(0,0,0,0),2)
...
class SafariDataBoxEBDX < SpriteWrapper
  ...
  def refresh
    ...
    pbDrawOutlineText(self.bitmap,0,38,self.bitmap.width,self.bitmap.height,str,Color.white,Color.new(0,0,0,0),1)
  end
  #-----------------------------------------------------------------------------
  #  update (temp)
  #-----------------------------------------------------------------------------
